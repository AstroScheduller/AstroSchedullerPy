...
def coord(psrName, retList = False):
    if(findSource(psrName.replace("PSR", "").replace(" ", "")) != False):
        ...
        print("[NOTICE@coord]", psrName, "is not specified in the SourceDB. Trying to solve on SIMBAD and INTERNET REQUIRED.")
...
def progressBar(i, a, length=20):
    ...
    return str(percentage) + "%" + "[" + strBar + "] (" + displayTime(estTime) + " remains)"
...
possibilities = list()
for i in range(len(psrList)):
    print("\r" + progressBar(i+1, len(psrList)) + " > [SORTING " + str(i+1) + "/" + str(len(psrList)) + "] Starting with " + psrList[i]['source'], end="")
...
def findSource(srcName, db_file="sources.db"):
    for thisSource in loadSourceDB(db_file):
        if(srcName in thisSource['IDENTIFIER']):
            return [thisSource['RA'], thisSource['DEC']]
...
# Open file    
fileHandler  =  open  ("psr_list_raw.txt",  "r")
...
for  line in  listOfLines:
    thisLine = dict()
    thisLine[0] = line.strip()[0:9].replace(" ", "")
    thisLine[1] = line.strip()[9:26].replace(" ", "")
    if("@-" in thisLine[0]):
        psr_catalog.append(temp)
        temp = dict()
    elif(thisLine[0] == "RAJ" or thisLine[0] == "DECJ" or thisLine[0] == "PSRJ" or thisLine[0] == "PSRB"):
        temp[thisLine[0]] = thisLine[1]

database = list()
for thisPsr in psr_catalog:
    ID = [thisPsr['PSRJ']]
    if(("PSRB" in thisPsr) and ("PSRJ" in thisPsr)):
        ID = [thisPsr['PSRJ'], thisPsr['PSRB']]
    if(("RAJ" in thisPsr) and ("DECJ" in thisPsr)):
        database.append({"RA": thisPsr['RAJ'], "DEC": thisPsr['DECJ'], "IDENTIFIER": ID})

open("sources.db", "w+").write(json.dumps(database))
